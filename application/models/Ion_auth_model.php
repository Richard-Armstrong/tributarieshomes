...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	/* Gets array of email addresses to user ids
	*
	* @return int
	* @author Richard
	**/
	...
	/**
	 * Updates the User's User Level
	 * Returns true or false based on success
	 *
	 * @return bool
	 * @author Daniel Armstrong
	 **/
	public function update_user_level($id, $level) {
		// Grab the name of the User level if possible
		$this->load->model('nvp_codes_model');
		$display = $this->nvp_codes_model->get_nvp_display('User_Levels', $level);
		$return = FALSE;
		// Check if the level is valid before attempting to update the User's level
		if (!$display) {
			$this->set_error('invalid_user_level');
		} else if ($level < $this->session->userdata('user_level')) {
			$this->set_error('unavailable_user_level');
		} else {
			if ($this->db->update($this->tables['users'], array('level' => $level), array('id' => $id)))
				$return = TRUE;
		}
	...
	**/
	 ...
	 * @author aditya menon
	*/
	public function create_group($group_name = FALSE, $group_description = '', $group_company = 0) {
		// Throw an error if there is no group name
		if (!$group_name) {
			$this->set_error('group_name_required');
			return FALSE;
		}
		// Throw an error if the same group name exists within the same Company
		$existing_group = $this->db->get_where($this->tables['groups'], array('name' => $group_name, 'company' => $group_company))->num_rows();
		if ($existing_group !== 0) {
			$this->set_error('group_already_exists');
			return FALSE;
		}
		// Load data to create the group
		$data = array(
			'name'			=> $group_name,
	 ...
	 * @author aditya menon
	 **/
	public function update_group($group_id = FALSE, $group_name = FALSE, $group_description = '', $group_company = FALSE) {
		// Throw an error if no there is no group id
		if (empty($group_id))
			return FALSE;
		// Declare $data as an array before using
		$data = array();
		// Check if the group name is changing
		if (!empty($group_name)) {
			// Throw an error if the same group name exists within the same Company
			$existing_group = $this->db->get_where($this->tables['groups'], array('name' => $group_name, 'company' => $group_company))->row();
			if (isset($existing_group->id) && $existing_group->id != $group_id) {
				$this->set_error('group_already_exists');
				return FALSE;
			}
			// Load the new group name to update
			$data['name'] = $group_name;
		}
		// Prevent changing the name of the admin group
		...
		// Prevent changing the name of the unassigned group
		if ($this->config->item('unassigned_group', 'ion_auth') === $group->name && $group_name !== $group->name) {
			$this->set_error('group_name_unassigned_not_alter');
			return FALSE;
		}
		// Check if the group description is changing
		if (!empty($group_description)) {
			// Load the new group description to update
			$data['description'] = $group_description;
		}
		// Check if the group company is changing
		if (!empty($group_company)) {
			// Throw an error if someone other than a superuser tries to alter another Company's Department
			if ($this->ion_auth->user()->company != $group_company && !is_superuser($this->session->userdata('user_level'))) {
				$this->set_error('group_company_incorrect');
				return FALSE;
			}
			// Load the new group description to update
			$data['company'] = $group_company;
		}
		// Update the group
