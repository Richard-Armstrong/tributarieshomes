...
<script>
	...
	async function finish_add() {
		...
		const response = await fetch('<?= base_url("index.php/api/form_logic_add") ?>', {
			method: 'POST',
	...
	async function delete_column(id) {
		if (confirm("This will delete this calculated column from the form! Do you wish to do this?")) {
			var data = {};
			data['form_id'] = <?= $form->id ?>;
			data['id'] = id;
			const response = await fetch('<?= base_url("index.php/api/form_logic_delete") ?>', {
				method: 'POST',
				body: JSON.stringify(data),
				headers:{
					'Content-Type': 'application/json'
