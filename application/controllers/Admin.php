...
class Admin extends MY_Controller {
	...
	public function inventory() {
		// Get list of all bios from model and display
		...
		$data['records'] = $this->inventory_model->get_all();
	...
	public function edit_inventory($id) {
		...
		$this->form_validation->set_rules('inv_name', 'Inventory Name', 'required|max_length[' . INVENTORY_NAME_MAX_LENGTH . ']');
		// Based on validation, load views or create new Company
		if ($this->form_validation->run() === FALSE) {
			// Load Company record
			$this->load->model('inventory_model');
			$data['record'] = $this->inventory_model->get($id);
			// Load array of Company IDs the Company has API access to

			// Load checked flag
			$data['active_flag'] = '';
			if ($data['record']->active_flag == 1) {
				$data['active_flag'] = 'CHECKED';
			}
			...
			// Pull form data
			$inv_name = trim($this->input->post('inv_name'));
			$inv_directory = trim($this->input->post('inv_directory'));
			$inv_description = trim($this->input->post('inv_description'));
			$inv_desc_short = trim($this->input->post('inv_desc_short'));
			$seq = trim($this->input->post('seq'));
			$active_flag = trim($this->input->post('active_flag'));
			$landing_image = trim($this->input->post('landing_image'));
			$flythru_link = trim($this->input->post('flythru_link'));


			if (strcmp($active_flag, "on") == 0 ) {
				$active_flag = 1;
			} else {
				$active_flag = 0;
			}

			// Load data to create the inventory
			$data = array(
				'inv_name'		  	=> $inv_name,
			...
			if ($this->inventory_model->update($id, $data)) {
				...
				// Reload the page
				$this->session->set_flashdata('message', "Unable to edit Inventory");
				redirect ("admin/edit_inventory/{$id}", 'refresh');
			}
		}
	}
	...
	public function bios() {
		...
		$data['records'] = $this->bios_model->get_all();

		// Load views
		$this->load->view('includes/admin_header', $data);
		$this->load->view('admin/bios/list', $data);
		$this->load->view('includes/admin_footer', $data);
	}
	...
	public function add_bio() {
		...
		// Load views
		$this->load->view("includes/admin_header", $data);
		$this->load->view("admin/bios/add", $data);
		$this->load->view("includes/admin_footer", $data);
	}

	public function edit_bio($bio_id) {
		// Check if the User has access to this page - Superuser or Account Manager accessing own Company
		if (!is_superuser($this->session->userdata('user_level')))
			return show_error("You lack permission to access this page.");

		// Set validation rules
		$this->form_validation->set_rules('bio_name', 'Bio Name', 'required|max_length[' . BIO_NAME_MAX_LENGTH . ']');
		// Based on validation, load views or create new Company
		if ($this->form_validation->run() === FALSE) {
			// Load Company record
			$this->load->model('bios_model');
			$data['record'] = $this->bios_model->get($bio_id);
			// Load array of Company IDs the Company has API access to
			...
			// Load views
			$this->load->view("includes/admin_header", $data);
			$this->load->view("admin/bios/edit", $data);
			$this->load->view("includes/admin_footer", $data);
		} else {
			// Pull form data
			$bio_name = trim($this->input->post('bio_name'));
			$bio_title = trim($this->input->post('bio_title'));
			$bio_companies = trim($this->input->post('bio_companies'));
			$bio_title = trim($this->input->post('bio_title'));
			$bio_image= trim($this->input->post('bio_image'));
			$bio_seq = trim($this->input->post('bio_seq'));
			$bio_description = trim($this->input->post('bio_description'));
			...
			// Attempt to update the Company and decide where to go and what to report based on success or failure
			$this->load->model('bios_model');
			if ($this->bios_model->update($bio_id, $data)) {
				$this->session->set_flashdata('message', "Bio updated");
				redirect ('/admin/bios');
			} else {
				// Reload the page
				$this->session->set_flashdata('message', "Unable to edit Bio");
				redirect ("admin/edit_bio/{$bio_id}", 'refresh');
			}
		}
	}
